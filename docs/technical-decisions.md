...
首版有五类模式：`idle`、`dragging`、`clickReaction`、`docking`、`docked`。状态优先级固定，事件数量有限。
...
状态机语义 click
        ↓ bindings
资源动作名 click-reaction
        ↓ animations
图集行、帧数、时长、循环、静态帧
```

代价是整张 8192×2560 图集解码后会占用一块固定内存，且所有帧必须统一单元格、基线和透明边界。对单只首版 Pet 可接受；若未来支持大量皮肤同时驻留，需要按需加载或拆分图集。
