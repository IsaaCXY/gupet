...
```text
idle ──指针左右──> pointerLook
 │                    │
 ├──单击──────────> clickReaction ──动画结束──┐
 │                                             │
 └──拖动──────────> dragging ──松手───────────┤
                                  │            │
...
```text
dragging > clickReaction > docking > docked > pointerLook > idle
```

状态机不硬编码资源名称。例如，状态机只请求 `pointerLeft`，实际播放 `look-left` 由 `pet.json.bindings.pointerLeft` 决定。替换动作命名或美术资源时，不需要修改状态转换逻辑。
...
左右追随使用角色可见边界，而不是完整 256×256 单元格。这样可避免素材周围透明留白导致视觉位置和触发位置不一致。
...
默认图集是 16 列、10 行、每格 256×256 的透明无损 WebP。十个动作分别占一行：

| 语义 | 默认动作 | 帧数 | 播放方式 |
|---|---|---:|---|
| 普通待机 | `idle` | 12 | 循环 |
| 向左/右看 | `look-left/right` | 各 8 | 循环 |
